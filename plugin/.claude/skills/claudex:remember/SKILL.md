...
- `/claudex:remember We use Vitest for testing, not Jest` → saves as **preference** with tags `[testing, vitest]`
- `/claudex:remember The auth service rate limits to 100 req/min` → saves as **fact** with tags `[auth, rate-limit]`
- `/claudex:remember Decided to use SQLite instead of PostgreSQL for simplicity` → saves as **decision** with tags `[database, sqlite]`
