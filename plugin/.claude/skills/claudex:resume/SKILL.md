...
- `/claudex:resume list` → shows all stashed conversations
- `/claudex:resume search auth` → finds stashed conversations about authentication
- `/claudex:resume API refactoring` → directly resumes the best-matching stash
- `/claudex:resume` → interactive picker if multiple stashes exist
