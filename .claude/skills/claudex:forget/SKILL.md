...
- `/claudex:forget SQLite migration` → searches, shows matches, confirms, then deletes
- `/claudex:forget obs_abc123` → confirms deletion of specific observation, then deletes
- `/claudex:forget everything before January` → uses `memory_forget` with `before_date` parameter
