...
function getConfig() {
  ...
  const settingsPath = import_node_path.default.join(dataDir, "settings.json");
  let userConfig = {};
  try {
    if (import_node_fs.default.existsSync(settingsPath)) {
      userConfig = JSON.parse(import_node_fs.default.readFileSync(settingsPath, "utf-8"));
    }
  } catch {
  }
  cachedConfig = {
...
function ensureDataDir() {
  ...
  if (!import_node_fs.default.existsSync(config.dataDir)) {
    import_node_fs.default.mkdirSync(config.dataDir, { recursive: true });
  }
...
function isPrivacyExcluded(filePath) {
  ...
  return config.privacy.excludePatterns.some(
    (pattern) => lower.includes(pattern.toLowerCase())
  );
}
var import_node_fs, import_node_path, import_node_os, DEFAULT_CONFIG, cachedConfig;
...
var import_node_fs2, import_node_path2, LEVELS, logLevel, logFile;
...
function runMigrations(db) {
  try {
    ...
    const hasColumn = cols.some((c2) => c2.name === "prior_claude_session_ids");
    if (!hasColumn) {
  ...
  try {
    const knCols = db.prepare("PRAGMA table_info('knowledge')").all();
    const hasSourceKnIds = knCols.some((c2) => c2.name === "source_knowledge_ids");
    if (!hasSourceKnIds) {
...
function initializeVectorTable(db) {
  try {
    db.exec(`
...
var log, SCHEMA_VERSION, SCHEMA_SQL, INDEXES_SQL, FTS_SQL, FTS_TRIGGERS_SQL;
...
function loadSqliteVec(db) {
  try {
    ...
    sqliteVec.load(db);
    log2.info("sqlite-vec extension loaded");
  } catch {
    log2.warn("sqlite-vec extension not available \u2014 vector search disabled");
  }
}
function getDb() {
  if (dbInstance) return dbInstance;
  const dbPath = getDbPath();
  log2.info("Opening database", { path: dbPath });
...
// src/db/sessions.ts
function createSession(projectId, claudeSessionId) {
  const db = getDb();
  const now = Date.now();
  ...
  log8.info("Created session", { id, projectId, claudeSessionId });
  return {
    id,
...
function endSession(id, summary, keyActions, filesModified) {
  const db = getDb();
  const now = Date.now();
  db.prepare(`
  ...
  log8.info("Ended session", { id });
}
function incrementSessionObservationCount(id) {
  const db = getDb();
...
var init_sessions = __esm({
  "src/db/sessions.ts"() {
    ...
    init_database();
    init_logger();
    log8 = createLogger("db:sessions");
  }
});

// src/db/conversations.ts
function createConversation(sessionId, projectPath, topic) {
  const db = getDb();
  const now = Date.now();
  ...
  log9.info("Created conversation", { id, sessionId, topic });
  return {
    id,
...
function stashConversation(id, stashGroupId) {
  const db = getDb();
  const now = Date.now();
  db.prepare(`
  ...
  log9.info("Stashed conversation", { id, stashGroupId });
}
function completeConversation(id, summary) {
  const db = getDb();
  const now = Date.now();
  db.prepare(`
...
function incrementConversationObservationCount(id) {
  const db = getDb();
  db.prepare("UPDATE conversations SET observation_count = observation_count + 1 WHERE id = ?").run(id);
}
function getStashedConversations(projectPath) {
  const db = getDb();
...
var init_conversations = __esm({
  "src/db/conversations.ts"() {
    ...
    init_database();
    init_logger();
    log9 = createLogger("db:conversations");
  }
});
...
function createKnowledge(input) {
  ...
  log10.info("Created knowledge", { id, type: input.type });
  return {
    id,
    type: input.type,
...
function getKnowledgeForProject(projectPath, limit = 50) {
  const db = getDb();
  const rows = db.prepare(
...
var init_knowledge = __esm({
  "src/db/knowledge.ts"() {
    ...
    init_database();
    init_logger();
    log10 = createLogger("db:knowledge");
  }
});
...
function createObservation(input) {
  ...
  `).run(
    ...
    input.conversation_id || null,
    input.tool_name,
    input.tool_input_summary || null,
    input.tool_output_summary || null,
    input.project_path,
    JSON.stringify(input.files_involved || []),
    JSON.stringify(input.tags || []),
    now
  );
  log13.debug("Created observation", { id, tool: input.tool_name });
  return {
    id,
...
function getObservationsBySession(sessionId, limit) {
  const db = getDb();
...
function getObservationsByConversation(conversationId, limit) {
  const db = getDb();
...
var init_observations = __esm({
  "src/db/observations.ts"() {
    ...
    init_database();
    init_logger();
    log13 = createLogger("db:observations");
  }
});
...
function storeEmbedding(sourceType, sourceId, text, embedding) {
  ...
  try {
    ...
    log19.debug("Stored embedding", { id, sourceType, sourceId });
    return true;
  } catch (err) {
    log19.error("Failed to store embedding", { error: err });
    return false;
  }
}
function searchByVector(queryEmbedding, limit = 20) {
  ...
  try {
    ...
    const rows = db.prepare(`
      ...
      LIMIT ?
    `).all(Buffer.from(embeddingBuffer), limit);
    return rows.map((row) => ({
      embedding_id: row.id,
    ...
    log19.error("Vector search failed", { error: err });
    return [];
  }
}
var import_node_crypto, log19;
...
async function loadFastEmbed() {
  ...
  try {
    ...
    log20.info("fastembed module loaded successfully");
  } catch (err) {
    log20.warn("fastembed not available \u2014 local embeddings disabled", { error: String(err) });
    FastEmbedModule = null;
  }
  return FastEmbedModule;
}
var log20, FastEmbedModule, fastEmbedLoadAttempted, AnthropicEmbeddingProvider, anthropicEmbeddings;
var init_anthropic = __esm({
  "src/embeddings/anthropic.ts"() {
    "use strict";
    init_logger();
    log20 = createLogger("embeddings:fastembed");
    FastEmbedModule = null;
    fastEmbedLoadAttempted = false;
    AnthropicEmbeddingProvider = class {
      dimensions = 384;
      _available = null;
      model = null;
      initPromise = null;
      ...
      async ensureModel() {
        ...
        this.initPromise = (async () => {
          try {
            ...
            log20.info("fastembed model initialized (BGE-small-en-v1.5, 384d)");
          } catch (err) {
            log20.error("Failed to initialize fastembed model", { error: String(err) });
            this._available = false;
            this.model = null;
          }
        })();
      ...
      async embed(texts) {
        ...
        try {
          ...
          log20.error("fastembed embed() failed", { error: String(err), textCount: texts.length });
          throw err;
        }
        if (results.length !== texts.length) {
          log20.warn("Embedding count mismatch", { expected: texts.length, got: results.length });
    ...
    anthropicEmbeddings = new AnthropicEmbeddingProvider();
  }
});
...
async function processQueue(batchSize) {
  ...
  if (!anthropicEmbeddings.available) {
    log21.debug("Embedding provider not available, skipping queue processing");
    return 0;
  }
  const db = getDb();
  ...
  if (pendingItems.length === 0) {
    log21.debug("No pending items in embedding queue");
    return 0;
  }
  log21.info("Processing embedding queue", { count: pendingItems.length });
  const texts = pendingItems.map((item) => item.text_content);
  let embeddings;
  try {
    embeddings = await anthropicEmbeddings.embed(texts);
  } catch (err) {
    log21.error("Batch embedding generation failed", { error: err });
    const now2 = Date.now();
  ...
  let processed = 0;
  const now = Date.now();
  for (let i2 = 0; i2 < pendingItems.length; i2++) {
    const item = pendingItems[i2];
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : "Unknown error";
      log21.error("Failed to store embedding for queue item", { error: err, itemId: item.id });
      db.prepare(`
        UPDATE embedding_queue SET status = 'error', error_message = ?, processed_at = ? WHERE id = ?
...
function extractFilePaths2(text) {
  if (!text) return [];
  const pathPatterns = [
    /(?:^|\s|["'])([/~][\w./-]+\.\w+)/g,
    // Absolute paths
    /(?:^|\s|["'])((?:src|lib|test|hooks|dist)\/[\w./-]+)/g,
    // Relative project paths
    /"file_path"\s*:\s*"([^"]+)"/g,
    // JSON file_path fields
    /"path"\s*:\s*"([^"]+)"/g
    // JSON path fields
  ];
  ...
  for (const pattern of pathPatterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      const p2 = match[1];
      if (p2 && !p2.includes("node_modules") && !p2.includes(".git/")) {
        paths.add(p2);
      }
    }
  }
...
function summarizeObservations(observations) {
  ...
  for (const obs of observations) {
    ...
    for (const f2 of obs.files_involved) {
      allFiles.add(f2);
    }
  }
  const parts = [];
  const toolSummary = [...toolCounts.entries()].sort((a2, b2) => b2[1] - a2[1]).map(([tool, count]) => `${tool}(${count})`).join(", ");
...
function extractReadSummary(input) {
...
function extractBashSummary(input) {
  if (typeof input === "object" && input !== null) {
    const obj = input;
...
function extractSearchSummary(toolName, input) {
  if (typeof input === "object" && input !== null) {
    const obj = input;
...
function extractWebSummary(input) {
  if (typeof input === "object" && input !== null) {
    const obj = input;
...
function summarizeFileContent(output) {
...
function summarizeBashOutput(output) {
  const lines = output.split("\n");
  const errorLines = lines.filter(
    (l2) => /error|Error|ERROR|failed|FAILED|fatal|FATAL|warning|Warning/i.test(l2)
  );
...
function summarizeSearchOutput(output) {
  const lines = output.split("\n").filter((l2) => l2.trim());
  if (lines.length <= 10) return output;
  return lines.slice(0, 10).join("\n") + `
... (${lines.length} total matches)`;
}
function truncateIntelligent(text, maxLen) {
...
function searchFts(query, options) {
  ...
  if (typeFilter === "all" || typeFilter === "observations") {
    try {
      ...
      log22.warn("FTS search on observations failed", err);
    }
  }
  if (typeFilter === "all" || typeFilter === "knowledge") {
    try {
      ...
      log22.warn("FTS search on knowledge failed", err);
    }
  }
  if (typeFilter === "all" || typeFilter === "sessions") {
    try {
      ...
      log22.warn("FTS search on sessions failed", err);
    }
  }
  if (typeFilter === "all" || typeFilter === "conversations") {
    try {
      ...
      log22.warn("FTS search on conversations failed", err);
    }
  }
  return results;
}
async function hybridSearch(query, options, queryEmbedding) {
  const config = getConfig();
  const weights = config.search;
  const limit = options.limit || 20;
  const ftsResults = searchFts(query, options);
  const maxFtsRank = Math.max(...ftsResults.map((r2) => Math.abs(r2.rank)), 1);
  const ftsScoreMap = /* @__PURE__ */ new Map();
  for (const r2 of ftsResults) {
    ftsScoreMap.set(r2.id, 1 - Math.abs(r2.rank) / maxFtsRank);
  }
  const vectorScoreMap = /* @__PURE__ */ new Map();
  if (queryEmbedding && isVectorsAvailable()) {
    const vecResults = searchByVector(queryEmbedding, limit * 2);
    const maxDist = Math.max(...vecResults.map((r2) => r2.distance), 1);
    for (const r2 of vecResults) {
      vectorScoreMap.set(r2.source_id, 1 - r2.distance / maxDist);
    }
  }
  const allIds = /* @__PURE__ */ new Set([...ftsScoreMap.keys(), ...vectorScoreMap.keys()]);
  const now = Date.now();
  const dayMs = 24 * 60 * 60 * 1e3;
  const resultMap = /* @__PURE__ */ new Map();
  for (const r2 of ftsResults) {
    resultMap.set(r2.id, r2);
  }
  const scored = [];
  for (const id of allIds) {
    ...
    const finalScore = weights.ftsWeight * ftsScore + weights.vectorWeight * vecScore + weights.recencyWeight * recencyBonus + weights.projectAffinityWeight * projectAffinity;
    if (options.tags && options.tags.length > 0 && ftsResult) {
      const itemTags = JSON.parse(ftsResult.tags || "[]");
      if (!options.tags.some((t2) => itemTags.includes(t2))) continue;
    }
    scored.push({
      id,
...
var init_search = __esm({
  "src/db/search.ts"() {
    ...
    init_vectors();
    init_logger();
    log22 = createLogger("db:search");
  }
});

// src/sdk/conflict-detector.ts
var conflict_detector_exports = {};
__export(conflict_detector_exports, {
  buildConflictPrompt: () => buildConflictPrompt,
  detectConflict: () => detectConflict
});
async function detectConflict(observation) {
  const config = getConfig();
  ...
  try {
    ...
    const candidates = results.filter((r2) => r2.id !== observation.id);
    if (candidates.length === 0) return null;
    const best = candidates[0];
    if (best.score >= EXACT_DUPLICATE_THRESHOLD) {
      log23.debug("Exact duplicate detected, skipping silently", {
    ...
    if (best.score >= threshold) {
      log23.info("Memory conflict detected", {
        newId: observation.id,
        existingId: best.id,
        score: best.score,
    ...
    log23.debug("Conflict detection search failed", { error: String(err) });
    return null;
  }
}
function buildConflictPrompt(conflict, observation) {
...
var log23, EXACT_DUPLICATE_THRESHOLD, CONFLICT_THRESHOLD;
var init_conflict_detector = __esm({
  "src/sdk/conflict-detector.ts"() {
    ...
    init_config();
    init_logger();
    log23 = createLogger("sdk:conflict");
...
// src/sdk/summarizer.ts
var summarizer_exports = {};
__export(summarizer_exports, {
  sdkExtractKnowledge: () => sdkExtractKnowledge,
  sdkSummarizeConversation: () => sdkSummarizeConversation,
  sdkSummarizeSession: () => sdkSummarizeSession
});
async function sdkSummarizeSession(sessionId) {
...
${digest}`
    ...
    log26.debug("SDK query() summarization unavailable, using extractive", { error: String(err) });
  }
  return summarizeObservations(observations);
}
async function sdkSummarizeConversation(conversationId) {
...
${digest}`
      ...
      if (result?.text) {
        const summary2 = result.text.trim();
        updateConversationSummary(conversationId, summary2);
        return summary2;
      }
    }
  } catch (err) {
...
async function sdkExtractKnowledge(sessionId) {
  const observations = getObservationsBySession(sessionId);
...
${digest}`
    ...
    if (result?.text) {
      try {
        ...
        log26.debug("Failed to parse knowledge extraction JSON");
      }
    }
  } catch (err) {
    log26.debug("Knowledge extraction unavailable", { error: String(err) });
  }
  return [];
}
function buildDigest(observations) {
  const lines = [];
  for (const obs of observations.slice(0, 40)) {
    const parts = [obs.tool_name];
...
var init_summarizer2 = __esm({
  "src/sdk/summarizer.ts"() {
    ...
    init_logger();
    init_summarizer();
    log26 = createLogger("sdk:summarizer");
  }
});
...
function createEdge(input) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(id, input.from_id, input.to_id, input.relationship, input.strength ?? 1, now);
  log28.info("Created edge", { id, from: input.from_id, to: input.to_id, rel: input.relationship });
  return {
    id,
...
var init_knowledge_graph = __esm({
  "src/db/knowledge-graph.ts"() {
    ...
    init_logger();
    init_config();
    log28 = createLogger("db:knowledge-graph");
  }
});

// src/sdk/discovery-engine.ts
var discovery_engine_exports = {};
__export(discovery_engine_exports, {
  onKnowledgeCreated: () => onKnowledgeCreated
});
async function onKnowledgeCreated(knowledge) {
  const config = getConfig();
  ...
  try {
    ...
    const related = await hybridSearch(knowledge.content, {
      ...
      type: "knowledge",
      project: knowledge.project_path || void 0,
      limit: 5
    });
    ...
    log29.warn("Discovery engine failed", { error: String(err) });
  }
  return edges;
}
async function tryDiscoverFromCombinations(knowledge) {
  ...
  try {
    ...
    for (const disc of discoveries) {
      const sourceIds = [knowledge.id, ...disc.sourceIds.filter((id) => candidates.some((c2) => c2.id === id))];
      const newKn = createKnowledge({
        type: "discovery",
        content: disc.content,
        source_observation_ids: knowledge.source_observation_ids,
        project_path: knowledge.project_path || void 0,
      ...
      log29.info("Auto-discovered knowledge", { id: newKn.id, content: disc.content.slice(0, 80) });
    }
  } catch {
    log29.debug("Automatic discovery skipped (SDK query not available)");
  }
}
function buildDiscoveryPrompt(knowledge, candidates) {
  const items = [knowledge, ...candidates].map((k2, i2) => `[${i2 + 1}] (${k2.type}, id: ${k2.id}) ${k2.content}`).join("\n");
  return `Given these knowledge items:

${items}

Can any NEW insight, conclusion, or discovery be derived by combining two or more of these items? Only output discoveries that logically follow from combining the given items \u2014 do not speculate beyond what the data supports.
...
function parseDiscoveries(text) {
  try {
    ...
    return parsed.discoveries.filter(
      (d2) => typeof d2.content === "string" && d2.content.length > 0 && Array.isArray(d2.sourceIds) && typeof d2.confidence === "number" && d2.confidence > 0.3
    );
  } catch {
    return [];
  }
}
function inferRelationship(newKn, existing) {
...
var init_discovery_engine = __esm({
  "src/sdk/discovery-engine.ts"() {
    ...
    init_search();
    init_queue();
    log29 = createLogger("sdk:discovery");
  }
});

// src/mcp/tools/save.ts
var save_exports = {};
...
async function handleSave(args) {
  ...
  Promise.resolve().then(() => (init_discovery_engine(), discovery_engine_exports)).then(({ onKnowledgeCreated: onKnowledgeCreated2 }) => onKnowledgeCreated2(knowledge)).then((edges) => {
    if (edges.length > 0) {
      log30.info("Discovery engine created edges", { knowledgeId: knowledge.id, edgeCount: edges.length });
    }
  }).catch((err) => log30.debug("Discovery engine skipped", { error: String(err) }));
  return {
...
var init_save = __esm({
  "src/mcp/tools/save.ts"() {
    ...
    init_queue();
    init_logger();
    log30 = createLogger("mcp:save");
  }
});

// src/sdk/curation-agent.ts
var curation_agent_exports = {};
__export(curation_agent_exports, {
  curateObservations: () => curateObservations
});
async function curateObservations(buffer, project) {
  const config = getConfig();
  ...
  try {
    ...
    log31.warn("Curation agent failed, flushing buffer", err);
  }
  buffer.flush();
  return { kept: staged.length, discarded: 0, merged: 0, knowledgeExtracted: 0, actions: [] };
}
async function runCurationAgent(sdk, staged, project) {
  const lines = staged.map((s2, i2) => {
    const obs = s2.observation;
    return `[${i2}] ${obs.tool_name}: ${obs.tool_input_summary || ""} \u2192 ${(obs.tool_output_summary || "").slice(0, 100)}`;
  });
...
Only return the JSON, nothing else.`;
  ...
  try {
    ...
    for (let i2 = 0; i2 < staged.length; i2++) {
      if (!keepSet.has(i2) && !discardSet.has(i2)) {
        keepSet.add(i2);
      }
    }
    const actions = [];
    ...
    if (knowledge.length > 0) {
      try {
        const { handleSave: handleSave2 } = await Promise.resolve().then(() => (init_save(), save_exports));
        for (const k2 of knowledge) {
          await handleSave2({
            content: k2.content,
            type: k2.type,
            tags: k2.tags,
        ...
        log31.warn("Failed to persist extracted knowledge", err);
      }
    }
    return {
      kept: keepSet.size,
    ...
    log31.warn("Failed to parse curation result", err);
    return { kept: staged.length, discarded: 0, merged: 0, knowledgeExtracted: 0, actions: [] };
  }
}
function applyCurationResult(buffer, staged, result) {
  const keepIds = [];
  const discardIds = [];
...
var log31;
var init_curation_agent = __esm({
  "src/sdk/curation-agent.ts"() {
    "use strict";
    init_logger();
    init_config();
    log31 = createLogger("sdk:curation");
  }
});

// src/sdk/checkpoint.ts
...
async function createCheckpoint(claudeSessionId, label) {
  const db = getDb();
  const session = getSessionByClaudeId(claudeSessionId);
  if (!session) {
    log33.warn("No session found for checkpoint", { claudeSessionId });
    return null;
  }
  ...
  const snapshot = {
    ...
    conversation_ids: conversations.map((c2) => c2.id),
  ...
  log33.info("Checkpoint created", { id, sessionId, label });
  return {
    id,
    session_id: sessionId,
    label: label || null,
...
var init_checkpoint = __esm({
  "src/sdk/checkpoint.ts"() {
    ...
    init_observations();
    init_conversations();
    log33 = createLogger("sdk:checkpoint");
  }
});

// src/hooks/adapters/hook-runner.ts
init_database();
init_config();

// src/hooks/logic/session-start-logic.ts
init_database();

// src/db/recovery.ts
init_database();
init_logger();
var log3 = createLogger("db:recovery");
function writeJournalEntry(operation, tableName, recordId, payload) {
  const db = getDb();
...
function cleanupJournal(keepCount = 1e3, maxAgeMs = 24 * 60 * 60 * 1e3) {
  const db = getDb();
  ...
  if (result.changes > 0) {
    log3.info("Cleaned up journal entries", { deleted: result.changes });
  }
  return result.changes;
}
function replayPendingEntries() {
  const pending = getPendingJournalEntries();
  if (pending.length === 0) return 0;
  log3.info("Replaying pending journal entries", { count: pending.length });
  const db = getDb();
  let replayed = 0;
    ...
    try {
      ...
      if (entry.operation === "insert_observation") {
        ...
        db.prepare(`
          INSERT INTO observations (id, session_id, conversation_id, tool_name, tool_input_summary, tool_output_summary, project_path, files_involved, tags, timestamp)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(p2.id, p2.session_id, p2.conversation_id, p2.tool_name, p2.tool_input_summary, p2.tool_output_summary, p2.project_path, p2.files_involved, p2.tags, p2.timestamp);
        ...
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(p2.id, p2.type, p2.content, p2.source_observation_ids, p2.conversation_id, p2.project_path, p2.tags, p2.confidence, p2.created_at, p2.updated_at);
      } else if (entry.operation === "stash_conversation") {
        db.prepare("UPDATE conversations SET status = 'stashed', stashed_at = ?, ended_at = ? WHERE id = ?").run(payload.stashed_at, payload.ended_at, entry.record_id);
      } else if (entry.operation === "update_session") {
        db.prepare("UPDATE sessions SET summary = ?, key_actions = ?, files_modified = ?, ended_at = ? WHERE id = ?").run(payload.summary, payload.key_actions, payload.files_modified, payload.ended_at, entry.record_id);
      }
      commitJournalEntry(entry.id);
      replayed++;
      log3.info("Replayed journal entry", { id: entry.id, operation: entry.operation });
    } catch (err) {
      log3.error("Failed to replay journal entry", { id: entry.id, error: err });
...
// src/recovery/restore.ts
init_logger();
var log4 = createLogger("recovery:restore");
function runRecovery() {
  log4.info("Starting recovery check");
  let replayed = 0;
  try {
    replayed = replayPendingEntries();
    if (replayed > 0) {
      log4.info("Replayed pending journal entries", { replayed });
    } else {
      log4.info("No pending journal entries to replay");
    }
  } catch (err) {
    log4.error("Error during journal replay", { error: err });
  }
  let cleaned = 0;
  try {
    cleaned = cleanupJournal();
    if (cleaned > 0) {
      log4.info("Cleaned up old journal entries", { cleaned });
    }
  } catch (err) {
    log4.error("Error during journal cleanup", { error: err });
  }
  log4.info("Recovery complete", { replayed, cleaned });
  return { replayed, cleaned };
}

// src/projects/detector.ts
...
init_database();
init_logger();
var log5 = createLogger("db:projects");
function createProject(rootPath, name, stack = []) {
  const db = getDb();
  const now = Date.now();
  ...
  log5.info("Created project", { id, rootPath, name });
  return {
    id,
    root_path: rootPath,
    name,
...
function getProjectByPath(rootPath) {
  const db = getDb();
...
// src/projects/stack-detector.ts
var import_node_fs3 = __toESM(require("node:fs"), 1);
var import_node_path3 = __toESM(require("node:path"), 1);
init_logger();
var log6 = createLogger("stack-detector");
var NPM_FRAMEWORK_INDICATORS = {
  react: "React",
  "react-dom": "React",
  vue: "Vue",
...
function detectStack(projectRoot) {
  ...
  try {
    ...
    log6.warn("Error during stack detection", { projectRoot, error: String(err) });
  }
  const result = Array.from(stack).sort();
  log6.debug("Detected stack", { projectRoot, stack: result });
  return result;
}
function detectNpmFrameworks(projectRoot, stack) {
  try {
    const pkgPath = import_node_path3.default.join(projectRoot, "package.json");
    const raw = import_node_fs3.default.readFileSync(pkgPath, "utf-8");
    ...
    log6.debug("Could not read package.json for framework detection", { error: String(err) });
  }
}
function fileExists(root, ...segments) {
  try {
    const fullPath = import_node_path3.default.join(root, ...segments);
    return import_node_fs3.default.statSync(fullPath).isFile();
  } catch {
    return false;
  }
}
function dirExists(root, ...segments) {
  try {
    const fullPath = import_node_path3.default.join(root, ...segments);
    return import_node_fs3.default.statSync(fullPath).isDirectory();
  } catch {
    return false;
  }
}
function hasFileWithExtension(root, ext) {
  try {
    const entries = import_node_fs3.default.readdirSync(root);
...
// src/projects/detector.ts
var log7 = createLogger("project-detector");
var PROJECT_MARKERS = [
  ".git/",
  "package.json",
  "Cargo.toml",
...
function detectProjectRoot(cwd) {
  const resolved = import_node_path4.default.resolve(cwd);
  ...
  while (true) {
    if (hasProjectMarker(current)) {
      ...
      log7.debug("Detected project root", { cwd: resolved, root: current });
      return current;
    }
    const parent = import_node_path4.default.dirname(current);
    if (parent === current) {
      rootCache.set(resolved, resolved);
      log7.debug("No project marker found, falling back to cwd", { cwd: resolved });
      return resolved;
    }
    current = parent;
  }
}
function detectAndRegisterProject(cwd) {
  const root = detectProjectRoot(cwd);
  const name = import_node_path4.default.basename(root);
  const stack = detectStack(root);
  log7.info("Registering project", { root, name, stack });
  return getOrCreateProject(root, name, stack);
}
function hasProjectMarker(dir) {
  for (const marker of PROJECT_MARKERS) {
    try {
      const fullPath = import_node_path4.default.join(dir, marker.replace(/\/$/, ""));
      const stat = import_node_fs4.default.statSync(fullPath);
...
function estimateTokens(text) {
  if (!text) return 0;
  const words = text.split(/\s+/).filter((w2) => w2.length > 0);
...
init_logger();
var log11 = createLogger("context-builder");
...
function buildSessionSection(sessions, conversations, budget) {
  if (sessions.length === 0) return null;
  const lines = [];
  let usedTokens = 0;
  for (const session of sessions.slice(0, 5)) {
    const age = formatAge(session.started_at);
    const resumable = session.is_resumable && session.claude_session_id ? ` [resumable: claude --resume ${session.claude_session_id}]` : "";
    const header = `Last session (${age})${resumable}:`;
    usedTokens += estimateTokens(header);
    if (usedTokens > budget) break;
    lines.push(header);
    const sessionConvs = conversations.filter((c2) => c2.session_id === session.id);
...
function buildKnowledgeSection(knowledge, budget) {
  if (knowledge.length === 0) return null;
  const items = [];
  let usedTokens = 0;
  const header = "Key knowledge:";
  usedTokens += estimateTokens(header);
  for (const k2 of knowledge.slice(0, 10)) {
    const item = `  [${k2.type}] ${truncateToTokens(k2.content, 60)}`;
...
function buildStashSection(stashed, budget) {
  if (stashed.length === 0) return null;
  const lines = [`Stashed sidebars (${stashed.length}):`];
...
function buildRecentFilesSection(sessions, budget) {
  const allFiles = /* @__PURE__ */ new Set();
  for (const session of sessions) {
    for (const f2 of session.files_modified) {
      allFiles.add(f2);
    }
  }
...
init_config();
init_logger();
var log12 = createLogger("hook:session-start");
async function handleSessionStart(input) {
  ...
  const allConversations = recentSessions.flatMap((s2) => getSessionConversations(s2.id));
  const stashedConversations = getStashedConversations(project.root_path);
  const stashedSidebars = stashedConversations.map((c2) => ({
    conversation: c2,
    groupLabel: null
  }));
  ...
  const context = buildSessionStartContext({
    project,
    recentSessions: recentSessions.filter((s2) => s2.id !== session.id),
    conversations: allConversations,
    stashedSidebars,
    knowledge,
    maxTokens: config.maxContextTokens
  });
  log12.info("Session started", {
...
// src/hooks/logic/user-prompt-logic.ts
init_database();
init_sessions();

// src/conversations/grouper.ts
init_conversations();
init_observations();

// src/db/thresholds.ts
init_database();
init_logger();
var log14 = createLogger("db:thresholds");
var DEFAULT_ASK = 0.4;
var DEFAULT_TRUST = 0.85;
function getThresholds(projectId) {
  const db = getDb();
  const row = db.prepare("SELECT * FROM adaptive_thresholds WHERE project_id = ?").get(projectId);
  if (row) {
    return {
      project_id: row["project_id"],
      ask_threshold: row["ask_threshold"],
      trust_threshold: row["trust_threshold"],
      auto_stash_count: row["auto_stash_count"],
      false_positive_count: row["false_positive_count"],
      suggestion_shown_count: row["suggestion_shown_count"],
      suggestion_accepted_count: row["suggestion_accepted_count"],
      updated_at: row["updated_at"]
    };
  }
  const now = Date.now();
...
function recordAutoStash(projectId) {
  const db = getDb();
  db.prepare(`
...
function recordSuggestionShown(projectId) {
  const db = getDb();
  db.prepare(`
    UPDATE adaptive_thresholds
...
// src/conversations/detector.ts
var import_node_path5 = __toESM(require("node:path"), 1);
init_logger();
var log15 = createLogger("conversations:detector");
var FILE_PATH_RE = /(?:\/[\w.\-]+){2,}(?:\.[\w]+)?|(?:[A-Z]:\\[\w.\-\\]+)/g;
...
function extractDirectories(files) {
  ...
  for (const f2 of files) {
    const dir = import_node_path5.default.dirname(f2);
    if (dir !== f2 && dir !== ".") {
      dirs.add(dir);
    }
  }
...
function extractToolNames(observations) {
  return new Set(observations.map((o2) => o2.tool_name));
}
var WEIGHT_FILE_OVERLAP = 0.3;
var WEIGHT_DIR_PROXIMITY = 0.15;
var WEIGHT_TIME_GAP = 0.25;
var WEIGHT_TOOL_PATTERN = 0.15;
var WEIGHT_PROMPT_STRUCTURE = 0.15;
function scoreFileOverlap(contextFiles, activityFiles) {
  if (contextFiles.size === 0 && activityFiles.size === 0) return 0;
  if (contextFiles.size === 0 || activityFiles.size === 0) return 0.5;
  let overlap = 0;
  for (const f2 of activityFiles) {
    if (contextFiles.has(f2)) overlap++;
  }
  const overlapRatio = overlap / activityFiles.size;
  return 1 - overlapRatio;
}
function scoreDirectoryProximity(contextFiles, activityFiles) {
  ...
  let overlap = 0;
  for (const d2 of activityDirs) {
    if (contextDirs.has(d2)) overlap++;
  }
  const overlapRatio = overlap / activityDirs.size;
  return 1 - overlapRatio;
}
function scoreTimeGap(lastObservationTimestamp) {
  if (!lastObservationTimestamp) return 0;
  const gapMs = Date.now() - lastObservationTimestamp;
  const gapMinutes = gapMs / 6e4;
  if (gapMinutes < 0.5) return 0;
  if (gapMinutes < 2) return 0.1;
  if (gapMinutes < 5) return 0.25;
  if (gapMinutes < 10) return 0.5;
  if (gapMinutes < 30) return 0.75;
  return 1;
}
function scoreToolPatternChange(recentTools, newActivity) {
  if (recentTools.size === 0) return 0;
  const codeTools = /* @__PURE__ */ new Set(["Read", "Edit", "Write", "Bash", "Grep", "Glob"]);
  const researchTools = /* @__PURE__ */ new Set(["WebFetch", "WebSearch"]);
  const recentIsCode = [...recentTools].some((t2) => codeTools.has(t2));
  const recentIsResearch = [...recentTools].some((t2) => researchTools.has(t2));
...
function scorePromptStructure(newActivity) {
...
function scoreTopicShift(ctx) {
  const { recentObservations, newActivity } = ctx;
  const contextFiles = /* @__PURE__ */ new Set();
  for (const obs of recentObservations) {
    for (const f2 of obs.files_involved) {
      contextFiles.add(f2);
    }
    for (const f2 of extractFilePaths(obs.tool_input_summary || "")) {
      contextFiles.add(f2);
    }
  }
  const activityFiles = extractFilePaths(newActivity);
  const recentTools = extractToolNames(recentObservations);
  const lastTimestamp = recentObservations.length > 0 ? recentObservations[recentObservations.length - 1].timestamp : null;
  const signals = {
  ...
  const score = signals.fileOverlap * WEIGHT_FILE_OVERLAP + signals.directoryProximity * WEIGHT_DIR_PROXIMITY + signals.timeGap * WEIGHT_TIME_GAP + signals.toolPatternChange * WEIGHT_TOOL_PATTERN + signals.promptStructure * WEIGHT_PROMPT_STRUCTURE;
  let newTopic = null;
  if (score >= 0.35) {
    newTopic = deriveTopicLabel(newActivity, activityFiles);
  }
  log15.debug("Topic shift scored", {
    score: score.toFixed(3),
    signals: Object.fromEntries(
      Object.entries(signals).map(([k2, v2]) => [k2, v2.toFixed(2)])
    ),
    newTopic
  });
  return { score, signals, newTopic };
}
function deriveTopicLabel(activity, files) {
  if (files.size > 0) {
    const first = [...files][0];
    const parsed = import_node_path5.default.parse(first);
    return import_node_path5.default.join(import_node_path5.default.basename(parsed.dir), parsed.base);
  }
...
// src/conversations/grouper.ts
init_logger();
var log16 = createLogger("conversations:grouper");
var CONTEXT_WINDOW = 8;
async function handleTopicShift(sessionId, projectPath, projectId, newActivity) {
  let active = getActiveConversation(sessionId);
  if (!active) {
    active = createConversation(sessionId, projectPath);
    log16.info("No active conversation, created new", { id: active.id });
  ...
  log16.info("Topic shift: TRUST tier \u2014 auto-stashing", {
    score: result.score.toFixed(3),
    trustThreshold: thresholds.trust_threshold,
...
// src/hooks/logic/user-prompt-logic.ts
init_logger();
var log17 = createLogger("hook:user-prompt");
async function handleUserPrompt(input) {
  ...
  try {
    ...
    switch (result.action) {
      ...
      case "ask":
        log17.info("Topic shift suggestion shown", {
          sessionId: session.id,
          score: result.score.toFixed(3),
    ...
    log17.warn("Topic shift detection failed", err);
    return { suggestion: null, action: "ignore" };
  }
}

// src/hooks/logic/post-tool-use-logic.ts
init_database();
init_sessions();
init_conversations();
...
init_knowledge();
init_logger();
var log18 = createLogger("recovery:journal");
function withJournal(operation, tableName, recordId, payload, fn) {
  const journalId = writeJournalEntry(operation, tableName, recordId, payload);
  log18.debug("Journal entry written", { journalId, operation, tableName, recordId });
  try {
    const result = fn();
    commitJournalEntry(journalId);
    log18.debug("Journal entry committed", { journalId });
    return result;
  } catch (err) {
    failJournalEntry(journalId);
    log18.error("Journal entry failed", { journalId, operation, error: err });
    throw err;
  }
}
function journaledInsertObservation(input) {
  return withJournal(
    "insert_observation",
    "observations",
    "",
    // record id assigned inside createObservation; replay uses payload
    input,
    () => {
      const obs = createObservation(input);
      log18.info("Journaled observation created", { id: obs.id });
      return obs;
    }
  );
}
...
init_summarizer();
init_logger();
var log24 = createLogger("hook:post-tool-use");
async function handlePostToolUse(input, buffer) {
  ...
  const allFiles = [
    ...extractFilePaths2(inputStr),
    ...extractFilePaths2(toolResponse)
  ];
  const filteredFiles = allFiles.filter((f2) => !isPrivacyExcluded(f2));
  if (allFiles.length > 0 && filteredFiles.length === 0) {
    log24.debug("All files excluded by privacy filter", { toolName });
    return null;
  }
  if (isPrivacyExcluded(inputSummary) || isPrivacyExcluded(outputSummary)) {
    log24.debug("Content excluded by privacy filter", { toolName });
    return null;
  }
  const tags = [];
  ...
  try {
    ...
    if (buffer) {
      ...
      Promise.resolve().then(() => (init_conflict_detector(), conflict_detector_exports)).then(({ detectConflict: detectConflict2 }) => detectConflict2(obs)).then((conflict) => {
        if (conflict && conflict.level === "similar" && !conflict.resolved) {
          buffer.flagConflict(staged.bufferId, conflict);
        }
      }).catch((err) => log24.debug("Conflict detection failed", { error: String(err) }));
    }
    log24.debug("Observation recorded", { id: obs.id, tool: toolName });
    return obs;
  } catch (err) {
    log24.error("Failed to record observation", err);
    return null;
  }
}

// src/hooks/logic/pre-compact-logic.ts
init_database();
init_sessions();
init_conversations();
init_logger();
var log25 = createLogger("hook:pre-compact");
async function handlePreCompact(input) {
  ...
  try {
    journaledInsertObservation({
      ...
      tool_input_summary: `Context compaction at ${(/* @__PURE__ */ new Date()).toISOString()}`,
    ...
    log25.info("Compaction snapshot saved", { sessionId: session.id });
  } catch (err) {
    log25.error("Failed to save compaction snapshot", err);
  }
}

// src/hooks/logic/session-end-logic.ts
init_database();
init_sessions();
init_conversations();
init_observations();
...
// src/conversations/summarizer.ts
init_observations();
init_conversations();
init_logger();
var log27 = createLogger("conversations:summarizer");
var FILE_PATH_RE2 = /(?:\/[\w.\-]+)+(?:\.[\w]+)?|(?:[A-Z]:\\[\w.\-\\]+)/g;
async function summarizeConversation(conversationId) {
  ...
  try {
    const { sdkSummarizeConversation: sdkSummarizeConversation2 } = await Promise.resolve().then(() => (init_summarizer2(), summarizer_exports));
    const summary2 = await sdkSummarizeConversation2(conversationId);
  ...
  if (apiKey) {
    try {
      const summary2 = await llmSummarize(apiKey, observations);
      updateConversationSummary(conversationId, summary2);
      return summary2;
    } catch (err) {
      log27.warn("LLM summarization failed, using extractive fallback", {
        error: String(err)
      });
    }
  } else {
    log27.debug("No ANTHROPIC_API_KEY, using extractive summary");
  }
  const summary = extractiveSummary(observations);
  updateConversationSummary(conversationId, summary);
  return summary;
}
async function llmSummarize(apiKey, observations) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 256,
    system: "Summarize the following developer conversation observations into 2-4 concise sentences. Focus on what was done, which files were touched, and the outcome. Do not include preamble.",
    messages: [{ role: "user", content: digest }]
  });
  const text = response.content[0]?.type === "text" ? response.content[0].text : "";
  return text.trim() || extractiveSummary(observations);
}
function extractiveSummary(observations) {
  const parts = [];
  const allFiles = /* @__PURE__ */ new Set();
  for (const obs of observations) {
    for (const f2 of obs.files_involved) {
    ...
    const combined = [obs.tool_input_summary, obs.tool_output_summary].filter(Boolean).join(" ");
    const matches = combined.match(FILE_PATH_RE2);
    if (matches) {
      for (const m2 of matches) allFiles.add(m2);
    }
  }
  const allTags = /* @__PURE__ */ new Set();
  for (const obs of observations) {
    for (const t2 of obs.tags) allTags.add(t2);
  }
...
init_config();
init_logger();
var log32 = createLogger("hook:session-end");
async function handleSessionEnd(input, buffer) {
  if (!input.session_id) return;
  getDb();
  const session = getSessionByClaudeId(input.session_id);
  if (!session) return;
  ...
  if (buffer && config.curation?.enabled) {
    try {
      const { curateObservations: curate } = await Promise.resolve().then(() => (init_curation_agent(), curation_agent_exports));
      const staged = buffer.getStaged();
      if (staged.length >= (config.curation.minObservations ?? 5)) {
        const result = await curate(buffer, project);
        log32.info("Curation completed", {
          kept: result.kept,
      ...
      log32.warn("Curation failed, flushing buffer", err);
      buffer.flush();
    }
  } else if (buffer) {
    buffer.flush();
  ...
  for (const conv of conversations) {
    try {
      ...
      log32.warn("Failed to summarize conversation", { id: conv.id, error: err });
    }
  }
  endSession(
    session.id,
  ...
  try {
    ...
    if (processed > 0) {
      log32.info("Processed embedding queue", { processed });
    }
  } catch (err) {
    log32.warn("Embedding queue processing failed", err);
  }
  log32.info("Session ended", {
    sessionId: session.id,
...
// src/sdk/system-prompt.ts
init_sessions();
init_conversations();
init_knowledge();
init_config();
function buildSystemPromptContext(project) {
  const config = getConfig();
  const recentSessions = getRecentSessions(project.id, config.sessionHistoryDepth);
  const allConversations = recentSessions.flatMap((s2) => getSessionConversations(s2.id));
  const stashedConversations = getStashedConversations(project.root_path);
...
// src/hooks/adapters/hook-runner.ts
init_logger();
var log34 = createLogger("hook:adapter");
...
function isDestructiveBash(input) {
  ...
  return DESTRUCTIVE_PATTERNS.some((p2) => p2.test(command));
}
...
async function main() {
  ...
  switch (event) {
    ...
    case "SessionStart": {
      try {
        const r2 = await handleSessionStart(input);
        const context = buildSystemPromptContext(r2.project);
        result = { additionalContext: context };
      } catch (err) {
        log34.error("SessionStart failed", err);
      }
      break;
    }
    // ── UserPromptSubmit ────────────────────────────────────────────
    case "UserPromptSubmit": {
      try {
        const r2 = await handleUserPrompt(input);
        if (r2.suggestion) {
          result = { additionalContext: r2.suggestion };
        }
      } catch (err) {
        log34.error("UserPromptSubmit failed", err);
      }
      break;
    }
    // ── PostToolUse ─────────────────────────────────────────────────
    case "PostToolUse": {
      try {
        ...
        log34.error("PostToolUse failed", err);
      }
      break;
    }
    // ── PreToolUse ──────────────────────────────────────────────────
    case "PreToolUse": {
      ...
      if (input.session_id && config.checkpoints?.enabled && config.checkpoints?.autoForkBeforeDestructive) {
        if (isDestructiveBash(input.tool_input)) {
          try {
            const { createCheckpoint: createCheckpoint2 } = await Promise.resolve().then(() => (init_checkpoint(), checkpoint_exports));
            const cmd = String(input.tool_input?.["command"]).slice(0, 60);
            await createCheckpoint2(input.session_id, `auto-fork before: ${cmd}`);
            log34.info("Auto-checkpoint before destructive Bash", { command: cmd });
          } catch (err) {
            log34.warn("Auto-checkpoint failed", err);
          }
        }
      }
      break;
    ...
    case "PreCompact": {
      try {
        ...
        log34.error("PreCompact failed", err);
      }
      break;
    }
    // ── SessionEnd ──────────────────────────────────────────────────
    case "SessionEnd": {
      try {
        ...
        log34.error("SessionEnd failed", err);
      }
      break;
    }
    default:
...
main().catch((err) => {
  log34.error("Hook runner failed", err);
  process.exit(1);
});
...
*/
//# sourceMappingURL=hook-runner.cjs.map
